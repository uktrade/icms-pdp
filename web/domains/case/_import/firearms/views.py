...
@require_GET
@login_required
def view_user_import_certificate_file(request, application_pk, certificate_pk, file_pk):
    application: OpenIndividualLicenceApplication = get_object_or_404(
    ...
    return import_views.view_file(
        request, application, application.user_imported_certificates, file_pk
    )
...
@login_required
@permission_required("web.importer_access", raise_exception=True)
def delete_user_import_certificate(request, application_pk, certificate_pk):
    with transaction.atomic():
        application: OpenIndividualLicenceApplication = get_object_or_404(
            OpenIndividualLicenceApplication.objects.select_for_update(), pk=application_pk
        )
